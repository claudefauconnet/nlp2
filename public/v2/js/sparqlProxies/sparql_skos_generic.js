var Sparql_skos_generic = (function () {
        ...
        self.getNodeChildren = function (conceptId, options, callback) {
            ...
            var conceptsGraphUri = Config.ontologies[Config.currentOntology].conceptsGraphUri
            var url = Config.sparql_url + "?default-graph-uri=" + encodeURIComponent(conceptsGraphUri) + "&query=";// + query + queryOptions
            var queryOptions = "&should-sponge=&format=application%2Fsparql-results%2Bjson&timeout=20000&debug=off"
    ...
    if(options.conceptId){
        filter = "  ?concept =<"+options.conceptId+">";
            ...
            if(options.conceptsGraphUri)
                ...
                conceptsGraphUri=Config.ontologies[Config.currentOntology].conceptsGraphUri;
            var url = Config.sparql_url + "?default-graph-uri=" + encodeURIComponent(conceptsGraphUri) + "&query=";// + query + queryOptions
            var queryOptions = "&should-sponge=&format=application%2Fsparql-results%2Bjson&timeout=20000&debug=off"
            sparql.querySPARQL_GET_proxy(url, query, queryOptions, null, function (err, result) {
                if (err) {
