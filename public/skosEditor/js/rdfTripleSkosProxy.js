var rdfTripleSkosProxy = (function () {
    ...
    self.showNodeConceptsGraph = function (nodeId) {
        var query = "prefix skos: <http://www.w3.org/2004/02/skos/core#>prefix foaf: <http://xmlns.com/foaf/0.1/>prefix schema: <http://schema.org/>" +
            "SELECT distinct ?catId  ?subject  ?concept ?conceptLabelLower ?broader1 ?broader1Label ?broader2 ?broader2Label ?broader3 ?broader3Label ?broader4 ?broader4Label WHERE{" +
            "  ?catId foaf:topic ?subject . " +
            "  ?category schema:about ?catId." +
            "  ?concept ?x ?category." +
            "  ?concept skos:prefLabel ?conceptLabel. filter(lang(?conceptLabel)='en')   " +
            "  filter ( ?catId=<" + nodeId + ">)BIND (LCASE(?conceptLabel) as ?conceptLabelLower)" +
            "  " +
            "  optional{ ?concept skos:broader ?broader1. ?broader1 skos:prefLabel ?broader1Label  filter(lang(?broader1Label)='en')  " +
            "    optional{ ?broader1 skos:broader ?broader2. ?broader2 skos:prefLabel ?broader2Label  filter(lang(?broader2Label)='en')  " +
            "    optional{ ?broader2 skos:broader ?broader3. ?broader3 skos:prefLabel ?broader3Label  filter(lang(?broader3Label)='en') " +
            "        optional{ ?broader3 skos:broader ?broader4. ?broader4 skos:prefLabel ?broader4Label  filter(lang(?broader4Label)='en') }" +
            "    }" +
            "    }" +
            "  }" +
            " " +
            ...
            result.results.bindings.forEach(function (item) {
              ...
              //  var schemeId = item.scheme.value;
               ...
               if (allnodes.indexOf(conceptId) < 0) {
                    ...
                    visjsData.nodes.push({id: conceptId, label: conceptLabel, shape: "box", color: color,fixed:{x:true,y:true},x:-500,y:offsetY})
                   offsetY+=30;
